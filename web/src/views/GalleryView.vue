  ...
  <main class="flex h-full min-h-0 flex-1 flex-col overflow-y-auto bg-background-dark">
    ...
    <div v-else class="px-4 pb-8 xs:px-6 md:px-8">
      ...
      >
        ...
        >
          <!-- Image -->
          <div :class="viewMode === 'grid' ? 'aspect-[4/5] overflow-hidden bg-gray-50' : 'h-20 w-20 shrink-0'">
            <img
              v-if="getImageUrl(item)"
              :src="getImageUrl(item)"
              :alt="item.prompt"
              :class="viewMode === 'grid' ? 'w-full h-full object-contain transition-transform group-hover:scale-105' : 'h-full w-full object-cover transition-transform group-hover:scale-105'"
              loading="lazy"
            />
