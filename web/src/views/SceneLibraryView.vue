    ...
    <div class="mx-auto mt-4 w-full max-w-[800px] px-4 pb-8 xs:px-6 md:px-8">
      ...
      <div v-else class="grid grid-cols-2 gap-4 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        <div
          ...
          :key="scene.id"
          @click="selectScene(scene)"
          class="group cursor-pointer overflow-hidden rounded-2xl border border-border-dark bg-white/80 transition hover:shadow-lg hover:border-primary/30"
        >
          <!-- Cover image / icon -->
          <div class="aspect-square overflow-hidden bg-primary-soft">
            <img
    ...
    >
      <template v-if="selectedScene">
        ...
        <div class="space-y-3">
          ...
          >
            <div v-if="tpl.exampleImage" class="aspect-[4/3] overflow-hidden">
              <img :src="tpl.exampleImage" class="h-full w-full object-cover transition-transform group-hover:scale-105" />
            </div>
            <div class="p-3">
...
// Scene data (will be loaded from API when backend is ready)
const categories = ref([
  { id: 'education', name: '教育智慧', icon: '📚' },
  { id: 'creative', name: '创意文案', icon: '✨' },
  { id: 'content', name: '内容创作', icon: '📝' },
...
onMounted(() => {
  // Merge admin-managed scenes with defaults
  try {
    const adminScenes = localStorage.getItem('admin_scenes')
    if (adminScenes) {
      const parsed = JSON.parse(adminScenes)
      if (parsed.length > 0) {
        // Merge: admin scenes override defaults by id, add new ones
