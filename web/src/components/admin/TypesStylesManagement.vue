  ...
  <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div v-for="(t, idx) in types" :key="idx" class="flex items-center gap-3 border border-border-dark rounded-xl p-3">
          ...
          <div class="w-16 h-16 shrink-0 rounded-lg overflow-hidden bg-primary-soft/20">
            ...
            <div v-else class="flex h-full items-center justify-center text-2xl">{{ t.emoji || '📁' }}</div>
          </div>
          <!-- 表单 -->
          <div class="flex-1 min-w-0 space-y-1.5">
            <div class="flex gap-2">
              <el-input v-model="t.label" size="small" placeholder="类型名称" class="!flex-1" />
              <el-input v-model="t.emoji" size="small" placeholder="Emoji" class="!w-16" />
            </div>
            <div class="flex gap-2">
              <el-input v-model="t.value" size="small" placeholder="英文KEY" class="!flex-1" />
              <el-input v-model="t.cover" size="small" placeholder="封面图URL" class="!flex-1" />
            </div>
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-border-dark p-6">
      ...
      <div class="flex flex-wrap gap-2">
        <div v-for="(s, idx) in styles" :key="idx" class="flex items-center gap-1 border border-border-dark rounded-lg px-2 py-1">
          <el-input v-model="styles[idx]" size="small" class="!w-20" />
          <el-button type="danger" size="small" text @click="styles.splice(idx, 1)" class="!p-0 !ml-0">
...
function addType() {
  types.value.push({ value: `type_${Date.now()}`, label: '', emoji: '📁', cover: '' })
}

function addStyle() {
  styles.value.push('新风格')
}
